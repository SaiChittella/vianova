...
import { CompleteIngredient, Ingredient } from "@/lib/types";

interface InventoryComponentProps {
	inventoryItems: CompleteIngredient[];
	isStaff: boolean;
}
							...
							<TableBody>
								{filteredItems && filteredItems?.length > 0 ? (
									filteredItems?.map((item) => {
										const quantity = countStock(item);
										let status = "In Stock";
										...
										if (item.medium_inventory_threshold) {
											if (quantity > item.low_inventory_threshold && quantity < item.medium_inventory_threshold) {
												status = "Medium Stock";
											}
										}
